...
const RoomPage = () => {
  ...
  useEffect(() => {
    const fetchPopularRooms = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/rooms/popular?count=6`);
    ...
    const fetchRecommendations = async () => {
      ...
      try {
        const response = await axios.get(
          `${API_BASE_URL}/rooms/recommendations/${user.id}?count=8`,
          {
  ...
  const recordInteraction = async (roomId, interactionType, additionalData = {}) => {
    ...
    try {
      await axios.post(
        `${API_BASE_URL}/rooms/interactions`,
        {
          userId: user.id,
          roomId,
          interactionType,
  ...
  const checkRoomAvailability = async (roomId, checkInDate, checkOutDate) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/rooms/availability`, {
        params: {
          checkInDate,
          checkOutDate
        }
